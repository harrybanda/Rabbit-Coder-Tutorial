...
By the end of this tutorial, you should have learned how to build a game with conplex game logic in SparkAR with javaScript.
...
From the code above we have imported the *Audio* module and added each `getAudioPlaybackController` to the promise, we have also assigned variables to them in the  `then` function. Now that we have access to all our Scene objects let's move on to the next step which is creating levels.
...
Looking for more ways to develop your Spark AR skills Check out the https://www.facebook.com/fbcameraeffects/tryit/208527660353698/[Official Spark AR Tutorials]. I used the following sites to obtain some of the game assets:

- https://www.kenney.nl/assets[Kenney Game Assets] 
